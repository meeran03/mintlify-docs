    ...
    },
    {
      ...
      "requirements": {
        ...
        "quality_requirements": {
          ...
          "bit_rate": 128,
          "channels": 1,
          "note": "Clean mono audio without background noise is recommended"
        }
      }
    }
  ]
...
| **Cartesia** | High | Fast | Usage-based | Low-latency multilingual cloned voices |

### Example Request

```bash cURL
curl -X GET "https://api.burki.dev/api/v1/assistants/123/voice-cloning/providers" \
...
def select_best_provider(audio_file_info):
    ...
    response = requests.get(
        "https://api.burki.dev/api/v1/assistants/123/voice-cloning/providers",
...
async function validateAudioFile(assistantId, audioFile) {
  try {
    ...
    const response = await axios.get(
      `https://api.burki.dev/api/v1/assistants/${assistantId}/voice-cloning/providers`,
      { headers: { 'Authorization': 'Bearer YOUR_API_KEY' } }
    );
