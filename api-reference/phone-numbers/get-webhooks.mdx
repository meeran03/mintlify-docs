...
openapi: 'GET /api/v1/phone-numbers/{phone_number}/webhooks'
---
...
```bash
GET /api/v1/phone-numbers/+14155551234/webhooks?provider=twilio
```
...
for number in "+14155551234" "+12125555678" "+13105559999"; do
  curl -H "Authorization: Bearer YOUR_API_KEY" \
    "https://api.burki.dev/api/v1/phone-numbers/${number}/webhooks"
done
```
...
async function getPhoneWebhooks(phoneNumber, provider = null) {
  try {
    ...
    const response = await axios.get(`https://api.burki.dev/api/v1/phone-numbers/${phoneNumber}/webhooks${params}`, {
      headers: {
...
def get_phone_webhooks(phone_number, provider=None):
    url = f"https://api.burki.dev/api/v1/phone-numbers/{phone_number}/webhooks"
...
function getPhoneWebhooks($phoneNumber, $provider = null) {
    $url = "https://api.burki.dev/api/v1/phone-numbers/" . urlencode($phoneNumber) . "/webhooks";
