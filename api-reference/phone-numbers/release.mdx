...
openapi: 'POST /api/v1/phone-numbers/release'
---
...
curl -X GET "https://api.burki.dev/api/v1/assistants/organization/phone-numbers" \
  -H "Authorization: Bearer YOUR_API_KEY"

# 2. Release a specific number
curl -X POST "https://api.burki.dev/api/v1/phone-numbers/release" \
  -H "Authorization: Bearer YOUR_API_KEY" \
...
async function releasePhoneNumber(phoneNumber, provider = null) {
  try {
    ...
    const response = await axios.post('https://api.burki.dev/api/v1/phone-numbers/release', requestData, {
      headers: {
...
def release_phone_number(phone_number, provider=None):
    url = "https://api.burki.dev/api/v1/phone-numbers/release"
    headers = {
        "Authorization": "Bearer YOUR_API_KEY",
        "Content-Type": "application/json"
    }
