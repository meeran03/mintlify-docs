---
title: "Live transcript WebSocket API — endpoint, auth & message formats | Burki Docs"
description: 'WebSocket API for real-time call transcript streaming, authentication, and message formats.'
sidebarTitle: Live transcript WebSocket API
"og:title": "Live transcript WebSocket API — endpoint, auth & message formats | Burki Docs"
"og:url": "https://docs.burki.dev/api-reference/websockets/live-transcript"
---
...
**Endpoint:** `wss://api.burki.dev/live-transcript/{call_sid}?token={api_key}`
...
**Required**: All connections must be authenticated with a valid API key or JWT token.

**Methods Supported:**

1. **Query Parameter** (Recommended):
   ```
   wss://api.burki.dev/live-transcript/{call_sid}?token=your_api_key
   ```

2. **Authorization Header** (non-browser clients):
   Use this with server-side WebSocket clients (Node `ws`, Python `websockets`, etc.). Browsers do not allow custom `Authorization` headers on `new WebSocket(...)`.
...
const websocket = new WebSocket(`wss://api.burki.dev/live-transcript/${callSid}?token=${apiKey}`);
