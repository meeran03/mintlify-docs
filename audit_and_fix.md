...
- 2026-04-30: Audited the frontend edit-assistant page and added missing assistant-setting coverage. Created `assistant-configuration.mdx` to map dashboard sections to runtime behavior and related docs; expanded Conversation Flows with base-assistant responsibilities and design patterns; documented cold vs warm transfers, warm-transfer confirmation/unavailable-message behavior, DTMF, SMS, speculative LLM/speculative execution controls, active listening/backchannels, incomplete utterance detection, conversation continuity, configurable recording/user-only tracks, bypass numbers, and updated assistant create/update API docs for these fields. Validation passed (`npx mintlify broken-links`, `npx mintlify validate`).
