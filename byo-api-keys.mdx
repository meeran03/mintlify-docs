...
At the organization level, Burki also stores managed-service preferences for LLM, TTS, STT, and carrier usage. These are exposed through the billing managed-services API (`GET`/`PATCH /v1/billing/managed-services`) and determine which service categories Burki manages for hybrid billing.

For organization-level API key, fallback key, and provider credential endpoints, see [Account and Organization API](/api-reference/account/overview). Assistant-level `byo_api_keys` and organization-level fallback keys are separate mechanisms: assistant keys override for a specific assistant, while fallback keys provide provider-level backup capacity for the organization.
...
| **Azure OpenAI** | LLM | `api_key`, `base_url`, deployment `model` |
| **Inworld** | TTS | `bearer_token`, `workspace_id` |
| **Resemble AI** | TTS | `api_key` |
| **Speechmatics** | STT | `api_key` |
| **Soniox** | STT, TTS | `api_key` |
| **Uplift** | STT, TTS | `api_key` |
| **Murf** | TTS | `api_key` |
| **Telnyx** | STT, Telephony, SMS | Telnyx API key / org carrier credentials |
...
See the [STT Providers & Settings](/stt-providers) guide for provider-specific model, endpointing, VAD, and silence controls.
...
Your API keys are protected with encryption and access controls:
...
1. Create or open an Azure Speech resource using the [Azure AI Speech quickstart](https://learn.microsoft.com/azure/ai-services/speech-service/get-started-speech-to-text)
...
1. Open your OpenAI Platform dashboard
2. Go to **API Keys**
