...
<section aria-labelledby="live-transcript-summary" className="mb-6">
  <h2 id="live-transcript-summary">Live transcript streaming</h2>
  <p>
    Connect over <strong>WebSocket</strong> to receive transcript segments while a call is still in progress. This complements REST endpoints that return transcripts after a call ends.
  </p>
  <ul>
    <li><strong>Authentication:</strong> use your Burki API key on the socket (query, header, or subprotocol as documented below).</li>
    <li><strong>Best for:</strong> live supervision, quality assurance, compliance alerts, and real-time captions.</li>
    <li><strong>Requires:</strong> a valid <code>call_sid</code> for an active or recent call your org can access.</li>
  </ul>
</section>
...
<section aria-labelledby="live-transcript-use-cases">
  <h2 id="live-transcript-use-cases">When to Use Live Transcripts</h2>
  <p>
    Live transcript streaming is useful when your application needs to react while a call is still active, not after the recording or post-call summary is complete.
  </p>
  <ul>
    <li>Show supervisors what callers and assistants are saying in real time.</li>
    <li>Trigger alerts when a transcript contains escalation phrases or compliance terms.</li>
    <li>Store timestamped segments for analytics, coaching, quality assurance, or captions.</li>
  </ul>
</section>
...
const wsUrl = `wss://api.burki.dev/live-transcript/${callSid}?token=${apiKey}`;
...
class LiveTranscriptClient {
    ...
    connect() {
        const wsUrl = `wss://api.burki.dev/live-transcript/${this.callSid}?token=${this.apiKey}`;
...
async def listen_to_transcript(call_sid, api_key):
    uri = f"wss://api.burki.dev/live-transcript/{call_sid}?token={api_key}"
...
const callSid = 'CA123abc...';
const apiKey = 'burki_your_api_key_here';
const ws = new WebSocket(`wss://api.burki.dev/live-transcript/${callSid}?token=${apiKey}`);
...
class LiveTranscriptClient extends EventEmitter {
    constructor(callSid, apiKey, host = 'api.burki.dev') {
        super();
        this.callSid = callSid;
        this.apiKey = apiKey;
...
<Card title="Need Help?" icon="question" href="mailto:support@burki.dev">
  Contact support for assistance with live transcript implementation
</Card>
