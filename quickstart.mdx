...
To make an authenticated API call, include the API key in the `Authorization` header of your request, prefixed with `Bearer `.

Here is an example using `curl` to list your assistants:
...
A successful request will return a JSON array of your assistants. Now you're ready to explore the full [API Reference](/api-reference/introduction)!
